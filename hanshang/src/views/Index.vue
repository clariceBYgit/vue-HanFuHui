<template>
  <div class="page-bg">
	...
	<!-- 返回顶部按钮 -->
  <div class="backTop" @click="toTop" id="top">
			<div></div>
  </div>
...
<style scoped>
		...
		.host_sale,.col_f,.col_m,.han_ele,.han_ornaments,.hot_brand{
			display:flex;
			width:1100px;
			height:750px;
			margin:0px auto;
		...
		.host_sale>div,.col_f>div,.col_m>div,.han_ele>div,.han_ornaments>div{
		width:248px;
		height:370px;
		...
		.host_sale>div>p,.col_f>div>p,.col_m>div>p,.han_ele>div>p,.han_ornaments>div>p{
	...
	.host_sale>div>p>i,.col_f>div>p>i,.col_m>div>p>i,.han_ele>div>p>i,.han_ornaments>div>p>i{	font-size: 18px;}
/*a标签的字体在鼠标移动上去是去掉下划线 */
	/* a:hover{
...
.backTop>div:hover{background: url(../../public/img/totop.png) 0 -50px;
transition: all .5s;
}
/* 楼层点亮 */
.affix{
...
.affix>a:last-child{border-bottom: 1px solid rgb(246, 204, 204);
...
.affix>a:nth-child(6){	background:#ff4466 url(../../public/img/user2.png) no-repeat 3px 18px;
	background-size: 30px 30px;}
.affix>a:nth-child(6):hover{
	 background: #fff url(../../public/img/user1.png) no-repeat 3px 18px;
	background-size: 30px 30px;
}
.affix a.active{background:rgb(145, 10, 10);
	color: #f7eeef;
}
.affix a:hover{background:#fff;
	color: #ff4466;
}
</style>